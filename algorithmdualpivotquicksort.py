...
def mySort(values):
 #Insert code here to sort the list "values"
    if len(values)<=1:
        return values
    pivot=[values[0],values[len(values)/3],values[2*len(values)/3],values[-1]]
    pivot.remove(max(pivot))
    pivot.remove(min(pivot))
    pivot1=[min(pivot)]
    pivot2=[max(pivot)]
    if pivot2[0]==pivot1[0]:
        if len(values)>values.index(pivot2[0])+1:
            pivot2=[values[values.index(pivot2[0])+1]]
        else:
            pivot2=[values[values.index(pivot2[0])-2]]
